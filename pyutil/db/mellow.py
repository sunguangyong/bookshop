...
class mellow:
    ...
    def RunSql(self, language, autocommit=False, out=list):
        ...
        '''
        action = language.split(" ")[0].upper()
        if action == "SELECT":
            return self.__excute_and_parse_datetime(language, out)
        elif action in ["UPDATE", "INSERT", "ALTER", "DELETE"]:
...
def test1():
    import pyconf.db.manna_dev as manna
    conf = {
        "host": "127.0.0.1",
        "port": 3366,
        "database": "fish_pole",
    ...
    print mysql.Find("appid_secret", ["appid"], limit=(2,2), out=dict)
    print "--------------------------------------------"
    time.sleep(1)
